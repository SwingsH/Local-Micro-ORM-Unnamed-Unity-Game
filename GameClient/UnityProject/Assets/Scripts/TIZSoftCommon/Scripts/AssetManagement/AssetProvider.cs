...
namespace TIZSoft
{
    ...
    public class AssetProvider
    {
        ...
        public void LoadAsync<T>(string category, string assetName, Action<T> onDone)
            where T : Object
        {
            LoadAsync(category, assetName, string.Empty, onDone);
        }
        ...
        /// <param name="onDone">On done.</param>
        /// <typeparam name="T">The type derived from UnityEngine.Object.</typeparam>
        public void LoadAsync<T>(string category, string assetName, string variant, Action<T> onDone)
            where T : Object
        {
            var assetBundleName = GetAssetBundleName(category, assetName, variant);
            var assetLoadingOperation = assetManager.LoadAssetAsync(assetBundleName, assetName, typeof(T));
            assetLoadingOperation.AssetLoaded +=
                asset => AssetManager_OnAssetLoaded(asset, category, assetBundleName, assetName, variant, onDone);
        }
            ...
            where T : Object
        {
            ...
            foreach (var fallbackAssetProvider in fallbackAssetProviders)
            {
                ...
                fallbackAssetProvider.LoadAsync<T>(category, assetName, variant, asset =>
                {
                    fallbackAsset = asset;
                    isResponded = true;
                });
        ...
        string GetAssetBundleName(string category, string assetName, string variant)
        {
            ...
            return string.Concat(entry.DirectoryName, assetBundleName);
        }
    }
}
