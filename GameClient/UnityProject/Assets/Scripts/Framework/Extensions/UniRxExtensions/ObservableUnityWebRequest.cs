...
namespace TIZSoft.UniRxExtensions
{
    ...
    public static class ObservableUnityWebRequest
    {
        ...
        /// <returns></returns>
        public static UniRx.IObservable<Texture2D> GetTexture(
            string url,
            bool nonReadable = false,
            UniRx.IProgress<float> uploadProgress = null,
            UniRx.IProgress<float> downloadProgress = null)
        {
        ...
        static IEnumerator Fetch(
            ...
            UniRx.IProgress<float> reportUploadProgress,
            UniRx.IProgress<float> reportDownloadProgress,
            System.Threading.CancellationToken cancellationToken) //CancellationToken maybe Ambiguous
        {
            using (request)
            {
                request.Send();
