...
#define Q9990 // UI不使用Atlas Nick 20180608
#define Q9996 // UnloadAssetBundle Nick 20180611
//#define Q9995 // 過濾只載劇情atlas Nick 20180612
...
namespace Tizsoft.AssetManagement
{
    ...
    public class AssetManager : IAssetManager
    {
...
#if Q9990 // UI不使用Atlas Nick 20180608 Nick 20180608
            // Nothing
#else
        ...
        {
#if Q9990 // UI不使用Atlas Nick 20180608
            // Nothing
#else
            // 
    #if Q9995 // 過濾只載劇情atlas Nick 20180612
            if (tag == "fungus_dialog_atlas")
			{
                // pass
			}
			else
			...
			{
				Scene getScene = UnitySceneManager.GetActiveScene();
				if (getScene == null)
				{
					return;
				}

				if (getScene.name == SceneNames.BeforeDialog)
				{
					return;
				}

				if (getScene.name == SceneNames.Launch)
				{
					return;
				}
				else
				if (getScene.name == SceneNames.Login)
				{
					return;
				}
				else
				if (getScene.name == SceneNames.Dialog)
				{
					return;
				}
			}
        ...
        void AssetOperateAsync_OnFinished(AssetLoadingOperation assetLoadingOperation)
       {
        ...
        public IObservable<ProgressValueSet> UpdateAllAsset()
        {
            if(updater != null)
