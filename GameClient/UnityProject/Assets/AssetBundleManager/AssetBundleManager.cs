//#define WEBREQUEST_ENABLE
#define B0147 //AssetBundleName 轉小寫,避免iOS平台找不到檔案
...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        bool m_SimulateAssetBundleInEditor;

        readonly Dictionary<string, LoadedAssetBundle> m_LoadedAssetBundles = new Dictionary<string, LoadedAssetBundle>();
        readonly Dictionary<string, string> m_DownloadingErrors = new Dictionary<string, string>();
        readonly List<string> m_DownloadingBundles = new List<string>();
        readonly List<AssetBundleLoadOperation> m_InProgressOperations = new List<AssetBundleLoadOperation>();
        ...
        public AssetBundleLoadManifestOperation Initialize(string manifestAssetBundleName)
        {
            ...
            var operation = new AssetBundleLoadManifestOperation(this, manifestAssetBundleName, "AssetBundleManifest", typeof(AssetBundleManifest));
        ...
        protected void LoadAssetBundle(string assetBundleName, bool isLoadingAssetBundleManifest)
        {
            Log(LogType.Info, "Loading Asset Bundle " + (isLoadingAssetBundleManifest ? "Manifest: " : ": ") + assetBundleName);

#if B0147 //AssetBundleName 轉小寫,避免iOS平台找不到檔案
        ...
        protected string GetAssetBundleBaseDownloadingURL(string bundleName)
        {
        ...
        protected bool UsesExternalBundleVariantResolutionMechanism(string baseAssetBundleName)
        {
        ...
        protected bool LoadAssetBundleInternal(string assetBundleName, bool isLoadingAssetBundleManifest)
        {
            ...
            {
                AssetBundleDownloadOperation downloadOperation = null;
                WWW www = null;
#if UNITY_5_3_OR_NEWER && WEBREQUEST_ENABLE
                ...
                var isFromStreamingAssets = bundleBaseDownloadingURL.StartsWith(GetStreamingAssetsPath());
                var isAssetBundleValid = true;

                // For manifest assetbundle, always download it as we don't have hash for it.
                if (isLoadingAssetBundleManifest)
                {
#if UNITY_5_3_OR_NEWER && WEBREQUEST_ENABLE
                    if (isFromStreamingAssets)
                    {
                        www = new WWW(url);
                    }
                    else
                    {
                        if (url.StartsWith(@"file:"))
                        {
                            ...
                            webRequest 
                                = UnityWebRequest.GetAssetBundle(string.Format("{0}?{1}", url, System.DateTime.Now.ToBinary()));
                    }
#else
                    if (isFromStreamingAssets)
                        www = new WWW(url);
                    else
                        www = new WWW(string.Format("{0}?{1}", url, DateTime.Now.ToBinary()));
#endif
                }
                else
                {
                    // TODO: 重構，將建立 WWW 和 UnityWebRequest 的流程抽出來。
                    // TODO: 不在 Manifest 裡就直接給 error，不要再讓 WWW 或 UnityWebRequest 去載。
                    var assetBundleHash = m_AssetBundleManifest.GetAssetBundleHash(assetBundleName);

                    isAssetBundleValid = assetBundleHash.isValid;
                    if (isAssetBundleValid)
                    {
#if UNITY_5_3_OR_NEWER && WEBREQUEST_ENABLE
                        if (isFromStreamingAssets)
                        {
                            www = assetBundleHash.isValid
                                ? WWW.LoadFromCacheOrDownload(url, assetBundleHash, 0)
                                : new WWW(url);
                        }
                        else
                        {
                            if (assetBundleHash.isValid)
                            {
                                ...
                                {
                                    webRequest =
                                        UnityWebRequest.GetAssetBundle(url, assetBundleHash, 0);
                                }
                                else
                                    webRequest =
                                        UnityWebRequest.GetAssetBundle(string.Format("{0}?{1}", url, assetBundleHash), assetBundleHash, 0);
                            }
                            else
                        ...
                        else
                        {
                            ...
                            ? WWW.LoadFromCacheOrDownload(
                                    string.Format("{0}?{1}", url, assetBundleHash),
                                    assetBundleHash,
                                    0)
                            : new WWW(url);
                        }
                    ...
                    {
                        downloadOperation = new NotExistAssetBundleDownloadOperation(assetBundleName, url);
                    }
                }
                ...
                if (isAssetBundleValid)
                {
                    if (www != null)
                    {
                        downloadOperation = new AssetBundleDownloadFromWwwOperation(assetBundleName, www);
                    }
#if UNITY_5_3_OR_NEWER && WEBREQUEST_ENABLE
                    if (webRequest != null)
                    {
                        downloadOperation = new AssetBundleDownloadFromWebRequestOperation(assetBundleName, webRequest);
                    }
#endif
                }
                m_InProgressOperations.Add(downloadOperation);
            }
        ...
        public AssetBundleLoadAssetOperation LoadAssetAsync(string assetBundleName, string assetName, System.Type type)
        {
            ...
            if (SimulateAssetBundleInEditor)
            {
                // 雖然 assetbundlname 只有小寫
                // 不過 GetAssetPathsFromAssetBundleAndAssetName 有分大小寫
                string[] assetPaths = AssetDatabase.GetAssetPathsFromAssetBundleAndAssetName(assetBundleName.Replace("\\", "/").ToLower(), assetName);
        ...
        public ICollection<string> GetAssetBundleDependencies(string assetBundleName)
        {
            string[] dependencies;
